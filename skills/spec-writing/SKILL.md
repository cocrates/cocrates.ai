...
2. **Strict Self-Containment Rule:** A reader (or `spec-driven-generation`) must understand **all decided requirements and constraints** from the Spec alone. ADRs explain *how* a decision was reached, but are **not** a dependency for building. 
   ...
   - **Prohibited:** Never use ADR hyperlinks as a substitute for actual specification bullets (e.g., Do not write `- Auth: See {project-root}/adr/auth.md`).
...
Before writing Specs, resolve **`{project-root}`** the same way as `spec-driven-generation` / `adr-writing`:
...
| **3** | Workspace groups projects by kind | `{kind}/{project-slug}/` |

**Rules:** Inspect workspace structure first; prefer an existing project folder; **confirm location and name with the user before creating** a new one; reuse if it already exists. If the folder does not exist yet, prefer routing through **`spec-driven-generation`** (which owns project-folder creation) or create `{project-root}/spec/` when the user explicitly asks to write a Spec first.

## Project Folder Layout

Specs are stored under `{project-root}/spec/`:
...
- **`{project-slug}`:** English **kebab-case** (same as `spec-driven-generation` / `adr-writing`). Used when Type 2/3 needs a folder name.
- **`{project-root}/spec/ASR.md`:** When encoding approved decisions, advance related ASRs to `approved` and record the Spec path under each ASR's **Spec** field.
- **`{project-root}/spec/PRD.md` (Product Requirement Definition):** The top-level master anchor that defines the high-level product boundaries, overarching goals, and global constraints.
- **`{project-root}/spec/{requirement-slug}.md`:** Modular, independent specifications for individual features or deliverables mapped to the PRD. Do not confuse with `ASR.md` (status registry) or `PRD.md` (product anchor).
- Place `spec/` under `{project-root}` only. Keep `ASR.md` only at `{project-root}/spec/ASR.md`.

### Before Creating a New Spec

Before saving, check the `{project-root}/spec/` directory.
...
- **Initial Setup:** If this is the start of a project, create **`{project-root}/spec/PRD.md`** first to capture the macro scope (ensure `{project-root}/` exists).
- **Feature/Component Focus:** If the PRD is established, map the current requirement to a specific `{requirement-slug}.md` file under `{project-root}/spec/`. Ensure it isolates a single, independent concern.
...
Save to `{project-root}/spec/{requirement-slug}.md` or `{project-root}/spec/PRD.md`.
After saving, update `{project-root}/spec/ASR.md` for each resolved ASR: set status to `approved` (when the user confirms), fill **Resolution** / **Spec** fields, and keep Summary in sync.

### 5. Notify and Route

Report the **file path** and a brief summary of what was captured or modified.
- *Example:* *"I have updated `{project-root}/spec/item-catalog-api.md` with the finalized cache requirements. The spec is now completely self-contained. ASR-003 is marked approved in `ASR.md`."*
...
- Writing Spec files outside `{project-root}/spec/`.
...
- Requirement scope isolated, and `{project-root}/spec/PRD.md` or functional `{project-root}/spec/{requirement-slug}.md` target determined.
- All approved ADR parameters deep-copied into the Spec, achieving **100% self-containment**.
- Related ASRs in `{project-root}/spec/ASR.md` advanced to `approved` with Spec path recorded.
