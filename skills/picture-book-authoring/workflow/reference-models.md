...
| **Staging** | **Who is where** relative to whom in a continuing multi-page scene | `stagings/{slug}.md` | `images/stagings/*` (typically **2–3** views) |
...
| Situation | What must not flip page-to-page |
|-----------|-------------------------------|
...
Then Stage ④ generates **2–3 reference images** of that ensemble (different useful framings of the **same** blocking), and later pages cite the staging + those images.
...
- Continuity span: {from page → to page} (or “until X leaves / reseats / situation ends”)
...
In episode page illustration guides, when the page belongs to a continuing situation:
...
Do not invent new seats, L/R flips, or OR station swaps in the page prompt. If the story needs a reseat or formation change, update Design (staging) first.
...
- [ ] Pages in that span cite the staging; expressions stay in cut direction
