...
**Not a numbered stage.** Canonical rules for what stays locked as reference images vs what is directed per cut. Read in Stage ② (catalog design), Stage ③ (integrity checks), and Stage ④ Phase 0 (reference generation).

**Shared model:** Same three layers as `picture-book-authoring`, `video-authoring`, and `novel-authoring` (character, location, staging). Continuity unit here = **cut**.

Reference models raise **visual consistency**. They are not the whole illustration — they are the durable identity and spatial anchors that page/cut generation must obey.
...
Expression, mood, transient pose, and **one-off** camera/time/weather are **not** reference models — direct them in cut Direction. Situation-spanning environment (rain through a café scene, etc.) may lock on **staging** (§3 / §4).
...
If `world-bible.md` defines species/tech classes (e.g. android vs humanoid, mage vs mundane), every character profile and cut Direction must obey that taxonomy. Do not mix class-forbidden traits (e.g. human facial acting on a class that has no face).
...
Every cut’s `- **Location:**` **must** be a catalog path — free-text place names are forbidden.
...
If the episode needs a new angle: **add a Scene List row** to `locations/{slug}.md` (and Phase 0 PNG) **before** citing it in the cut. G2/G3 must verify: every cut Location ⊆ some Scene List row.
...
1. The situation spans **≥2 cuts** in the same place/situation, **and**
2. Either **≥2 named characters** appear, **or** relative placement vs set fixtures matters (bench, table, counter, vehicle seats, …)

**`Staging: none` allowed only for:** single-cut establishing / empty set / no fixed relative placement needed. Long multi-cut ensemble scenes with `Staging: none` are a **hard fail** at G2/G3.
...
Staging’s job is to lock **ensemble placement in space** — not to catalog every cut camera.
...
| **`detail`** (optional — only to lock a **prop cluster**, not a CU face) | Registering every cut’s camera as a staging view |
...
**Phase 1 cuts:** Attach Character + Location + Staging **establishing** under `params.references`. Derive CU / OTS / tighter cameras in cut **Direction** — do **not** invent new staging refs for those angles.
...
Do not invent seats, L/R flips, or wholesale prop swaps in the cut prompt. Update Design first.
...
| Angry face / CU / OTS | Cut Direction (not a staging ref) |
...
| One-cut dusk grade | Cut Direction |
...
- [ ] No metadata labels in page prompts
...
| Staging missing for multi-cut ensemble | — | **Add** staging before locking cuts |

### Hard rules

- Do **not** invent outfit, seats, L/R, props, ambient, or free-text locations only inside the cut. Update catalogs first.
