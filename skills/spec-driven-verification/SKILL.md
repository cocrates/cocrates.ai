...
description: >-
  ...
  {project-root}/spec/ (including {project-root}/spec/PRD.md). Select when
...
Before verifying, resolve **`{project-root}`** the same way as `spec-driven-generation`:
...
**Rules:** Inspect workspace structure first; prefer an existing project folder; **confirm location and name with the user before creating** a new one; reuse if it already exists. Ask if ambiguous which project to verify.
...
- **`{project-slug}`:** English **kebab-case** name for the project. Used when Type 2/3 needs a folder name.
- **`{project-root}/spec/ASR.md`:** When undocumented ASRs are confirmed, register them here (`identified` / `reviewing`) before routing to Spec or ADR work.
...
Verification reads from **`{project-root}/spec/`**.
...
|-----------------|--------|
| Named file or slug (e.g. `{project-root}/spec/item-catalog-api.md`) | Verify against **`{project-root}/spec/PRD.md` AND that specific Spec** |
| "All specs", "everything in {project-root}/spec/" | Verify against **all** `{project-root}/spec/*.md` files including `PRD.md` |
| Unspecified | Ask one Socratic question: which `{project-slug}` and single deliverable vs multiple — then use the matching set |

**Do not read `{project-root}/adr/` files for verification.** If a Spec bullet references ADR content without stating the decided requirement, flag it as a **Spec gap** and recommend **`spec-writing`** to make the Spec self-contained before re-verifying.

---

## Output Location

Write every verification report to **`{project-root}/verification/`** (create the directory if missing).
...
- **Filename:** Same **kebab-case slug** as the governing Spec (e.g. `{project-root}/spec/item-catalog-api.md` → `{project-root}/verification/item-catalog-api.md`).
...
Clarify what to verify, which `{project-slug}` applies, and which Specs govern it. Always pull `{project-root}/spec/PRD.md` alongside the target spec to ensure high-level alignment.

### 2. Load Spec(s) & Extract Checklist
Read `{project-root}/spec/PRD.md` and the target `{project-root}/spec/*.md`. Extract every verifiable item into a working checklist:
...
Save the full report to **`{project-root}/verification/{requirement-slug}.md`** using the standardized template.
...
- If the user accepts a Deviation or confirms an Undocumented ASR: Register (or reopen) the ASR in `{project-root}/spec/ASR.md`, then load `spec-writing` to update `{project-root}/spec/PRD.md` or `{project-root}/spec/*.md` and mark the ASR `approved` once encoded.
...
- Writing verification reports outside `{project-root}/verification/`.
...
- Verification inventory mapped against both `{project-root}/spec/PRD.md` and detailed `{project-root}/spec/*.md`.
