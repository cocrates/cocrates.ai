...
description: >-
  Verifies generated deliverables against Spec documents in spec/ (including spec/PRD.md). 
  Select when the user asks to verify, validate, check, or confirm that an 
  artifact matches the specification; after spec-driven-generation completes; 
  or when reviewing whether output was produced according to the spec. 
  Performs item-by-item comparison, surfaces spec deviations, and flags 
  undocumented ASRs (Architecturally Significant Requirements) to drive the 
  iterative specification and refinement loop.
...
|-----------------|--------|
| Named file or slug (e.g. `spec/item-catalog-api.md`) | Verify against **`spec/PRD.md` AND that specific Spec** |
| "All specs", "everything in spec/" | Verify against **all** `spec/*.md` files including `PRD.md` |
...
Clarify what to verify and which Specs govern it. Always pull `spec/PRD.md` alongside the target spec to ensure high-level alignment.

### 2. Load Spec(s) & Extract Checklist
Read `spec/PRD.md` and the target `spec/*.md`. Extract every verifiable item into a working checklist:
...
Save the full report to **`verification/{requirement-slug}.md`** using the standardized template.

```markdown
# Verification: {Requirement Title}

**Spec:** spec/{requirement-slug}.md (Aligned with spec/PRD.md)
**Artifact(s):** {paths}
**Verified:** {ISO date}
...
- If the user accepts a Deviation or confirms an Undocumented ASR: Load `spec-writing` to immediately update `spec/PRD.md` or `spec/*.md`.
- If an Undocumented ASR involves complex trade-offs: Load `adr-writing` to evaluate alternative approaches, approve a choice, update the Spec, and then trigger regeneration.
...
- Verification inventory mapped against both `spec/PRD.md` and detailed `spec/*.md`.
