...
若已安装，可再次提出相同请求。AI 会按 [install.md](https://cocrates.ai/install.md) 以**升级**方式处理。

- **Plugin：** 比较已安装版本与最新版本。若有新版本则更新，并告知旧版→新版号；若已是最新则告知无需更新。
- **Skill：** 逐个比较官方仓库与本地文件。无差异则跳过；有差异则展示变更摘要，并请用户选择**替换为官方版本还是保留本地**。

无需记忆单独的「升级」命令，照常请求安装即可。
...
3. **让 AI 安装，然后验证。** 一行："Install based on install.md." 已安装时，相同请求也会触发升级。安装或升级后，确认 Plugin 与 Skill 就位。养成检查、审查 AI 工作的习惯。
