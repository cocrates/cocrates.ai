...
- **스토리라인**:
  ...
  2. **사용자 선언 7계명 — AI 주권(AI Sovereignty) 선언**:
     - *1. I do not blindly request.* — 맹목적으로 요청하지 않는다
     - *2. I do not blindly trust.* — 맹목적으로 신뢰하지 않는다
     - *3. I do not surrender control.* — 통제권을 포기하지 않는다
     - *4. I do not copy and run.* — 복사하고 실행하지 않는다
     - *5. I do not hide my ignorance.* — 내 무지를 숨기지 않는다
     - *6. I always reflect and learn.* — 항상 반성하고 배운다
     - *7. I always fight for my growth.* — 항상 성장을 위해 싸운다
  3. **선언문을 마음속 깊이 새긴다**: 단순히 읽고 넘어가는 것이 아니다. 의심될 때, 이 선언문으로 돌아온다. "지금 나는 이 선언문에 따라 행동하고 있는가?"
  4. **Cocrates Harness + User Manifesto = 무한한 역량 확장**: 도구(Cocrates Harness)와 의지(User Manifesto)가 하나가 될 때, 당신은 AI-native Engineer로 거듭난다
- **핵심 메시지**: Cocrates Harness의 궁극적인 목표는 사용자가 AI를 올바르게 활용하여 **자신의 능력을 극대화**하는 것이다. 사용자 선언 7계명은 이 여정의 나침반이다. **Cocrates Harness(도구) + User Manifesto(의지) = AI-native Engineer(무한한 역량 확장)**.
- **포함할 내용**:
