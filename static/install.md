...
This document describes how to install or upgrade **Cocrates Harness** on [OpenCode](https://opencode.ai). Use it when a user asks you to install Cocrates Harness: follow the steps below to configure the OpenCode plugin, register the required MCP servers (`cocrates-google-genai`, `cocrates-elk`, `cocrates-video-edit`), and copy the required skill files.
...
- All MCP servers run via `npx` (Node.js) on the machine
...
3. Ensure the `cocrates-google-genai`, `cocrates-elk`, and `cocrates-video-edit` MCP servers are configured in `opencode.jsonc`
...
| MCP configured | `OPENCODE_CONFIG/opencode.jsonc` contains `cocrates-google-genai`, `cocrates-elk`, and `cocrates-video-edit` under `mcp` |
...
Cocrates skills call MCP servers through OpenCode. Plugin + skills alone are not enough — `opencode.jsonc` must include all three:

| MCP server key | Package | Used by |
|----------------|---------|---------|
| `cocrates-google-genai` | `@cocrates/google-genai-mcp` | image, video, speech, music generation skills |
| `cocrates-elk` | `@cocrates/elk-mcp` | diagram-generation (Excalidraw, draw.io layout) |
| `cocrates-video-edit` | `@cocrates/video-edit-mcp` | video-authoring (edit-spec validate / render assemble) |

1. Open `OPENCODE_CONFIG/opencode.jsonc`.
2. If any server is missing or incomplete, **merge** the following into the existing config (do not remove other plugins, MCP servers, or settings):
...
3. Confirm the file still has:
   ...
   - `"mcp"."cocrates-elk"` with `type: "local"` and the `npx -y @cocrates/elk-mcp` command array above
   - `"mcp"."cocrates-video-edit"` with `type: "local"` and the `npx -y @cocrates/video-edit-mcp` command array above
...
4. Report to the user **for each server**:
   ...
   | Present (same) | Present | `MCP already configured: {server-key}` |
   | Present (different command) | Updated to match | `MCP updated: {server-key}` |

   Use `{server-key}` = `cocrates-google-genai`, `cocrates-elk`, or `cocrates-video-edit`.

Preserve any other entries under `mcp` when merging. OpenCode starts these servers via `npx` on demand; no separate global install of `@cocrates/google-genai-mcp`, `@cocrates/elk-mcp`, or `@cocrates/video-edit-mcp` is required.
...
4. Confirm `opencode.jsonc` still includes `mcp.cocrates-google-genai`, `mcp.cocrates-elk`, and `mcp.cocrates-video-edit` after restart (needed for media generation, diagram layout, and video-authoring assemble skills).

Installation or upgrade is complete when the user can select the **Cocrates** agent in OpenCode and `opencode.jsonc` has the plugin and all three MCP entries (`cocrates-google-genai`, `cocrates-elk`, `cocrates-video-edit`).
